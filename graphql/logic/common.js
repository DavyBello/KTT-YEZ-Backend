...
const updateSelf = exports.updateSelf = TC => TC.get('$updateById').wrapResolve(next => async (rp) => {
...
const isSelf = exports.isSelf = (TC, resolver) => TC.get(resolver).wrapResolve(next => async (rp) => {
  // get sourceUser from resolveParams (rp)
  const { args, sourceUser, sourceType } = rp;
  if (sourceUser._id == args.record._id) {
    const result = await next(rp);
...
const containSelf = exports.containSelf = (TC, resolver) => TC.get(resolver).wrapResolve(next => async (rp) => {
  // get sourceUser from resolveParams (rp)
  const { args, sourceUser, sourceType } = rp;
  if (sourceUser._id == args.record._id) {
    const result = await next(rp);
...
const findSelfRelationship = exports.findSelfRelationship = (field, TC) => TC.get('$findById').wrapResolve(next => async (rp) => {
  // get sourceUser from resolveParams (rp)
  const { args, sourceUser, sourceType } = rp;
...
const createSelfRelationship = exports.createSelfRelationship = (field, TC) => TC.get('$createOne').wrapResolve(next => async (rp) => {
  // get sourceUser from resolveParams (rp)
...
const updateSelfRelationship = exports.updateSelfRelationship = (field, TC) => TC.get('$updateById').wrapResolve(next => async (rp) => {
  // get sourceUser from resolveParams (rp)
  const { args, sourceUser, sourceType } = rp;
  const _field = sourceUser[field];
  if (Array.isArray(_field)) {
...
const deleteSelfRelationship = exports.deleteSelfRelationship = (field, TC) => TC.get('$removeById').wrapResolve(next => async (rp) => {
  // get sourceUser from resolveParams (rp)
  const { args, sourceUser, sourceType } = rp;
  if (sourceUser) {
    const _field = sourceUser[field];
...
const createManagedRelationship = exports.createManagedRelationship = (field, TC, managedModelType) => TC.get('$createOne').addArgs({
  managedId: 'String!',
  ...
  try {
    ...
    // console.log(e);
    if (e.message === `Unknown keystone list "${managedModelType}"`) { throw new Error(`Unknown model type "${managedModelType}"`); }

    if (e.message === `Cast to ObjectId failed for value "${managedId}" at path "_id" for model "${managedModelType}"`) { throw new Error(`Invalid Id supplied for model type "${managedModelType}"`); }
...
// Create and add id of relationship document (Cloudinary file) to the sourceUser/Self
const deleteManagedRelationship = exports.deleteManagedRelationship = (field, TC, managedModelType) => TC.get('$removeById').addArgs({
  managedId: 'String!',
  // managedModelType: 'String!'
}).wrapResolve(next => async (rp) => {
  // get sourceUser from resolveParams (rp)
...
const createModelRelationship = exports.createModelRelationship = (field, TC) => TC.get('$createOne').wrapResolve(next => async (rp) => {
  // get sourceUser from resolveParams (rp)
  const { sourceUser, sourceType } = rp;
  if (sourceUser) {
    const _field = sourceUser[field];
